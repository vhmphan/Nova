...
if __name__ == "__main__":
    ...
    dEbg=jnp.append(jnp.diff(Ebg), 0.0)[jnp.newaxis,:,jnp.newaxis]        # eV
    sigma_gg=func_sigma_gg(Eg, Ebg)[:,:,jnp.newaxis]                   # cm^2
    ...
    if(mytodo=='scan'):
        ...
        plot_LOPT(pars_nova)
        plot_flux_OPT(pars_nova, t)
        plot_tau_ph(pars_nova, tau_ph1, tau_ph2, Eg, t)
    ...
    # ax.plot(Eg,func_sigma_gg(Eg, Ebg)[:, 0],'g--',linewidth=3.0,label=r'${\rm Aharonian\, 2013}$')
    
    # # nu=me_g**2*(3.0e10)**4/(Eg*1.6e-12*k_B*pars_nova[11])
    # # sigma_gg_T09=45.0*sigma_Thomson_CGS/(8.0*np.pi**4.0)*f_nu(nu)
    # # sigma_gg_Peretti=func_sigma_Peretti(Eg, Ebg)
    
    # # ax.plot(Eg, sigma_gg_T09, 'r:', linewidth=3.0, label=r'${\rm Tatischeff\, 2009}$')
    # # ax.plot(Eg, sigma_gg_Peretti, 'k-.', linewidth=3.0, label=r'${\rm Peretti}$')

    # sigma_gg_G67=func_sigma_gg_G67(Eg, Ebg)
    # ax.plot(Eg, sigma_gg_G67[:, 0], 'm-.', label=r'${\rm Gould \, 1967}$')

    # sigma_gg_C90=func_sigma_gg_C90(Eg, Ebg)

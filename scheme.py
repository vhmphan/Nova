...
# Create a density map that scales as 1/R^2 centered on RG
density = 1 / (R**2 + 0.05)  # Added small term to avoid singularities

# Normalize the density map to [0, 1] range
density_normalized = density / np.max(density)

# Plot the density background (simulating RG wind)
ax.imshow(density_normalized, extent=(-1, 1, -1, 1), origin='lower', cmap='Reds', alpha=0.7)

# Define shock boundary radius
Rshock = 0.3
...
# Plot the hourglass shape (shock region)
ax.fill_between(x, y1, y2, color='skyblue', alpha=0.5)
...
# Add observer and line-of-sight shock geometry
theta = np.radians(30)  # Angle in degrees
s1 = np.array([0.4 + Rshock * np.cos(theta), Rshock * np.sin(theta)])  # Shock entry point
s2 = np.array([0.4 + Rshock * np.cos(theta) * 0.6, Rshock * np.sin(theta) * 0.6])  # Shock exit point
observer = np.array([0.8, 0])  # Observer location

# Plot the shock segment
ax.plot([s1[0], observer[0]], [s1[1], observer[1]], 'k-', lw=3, label="Shock")
ax.plot([s2[0], s1[0]], [s2[1], s1[1]], 'k-', lw=3)

# Plot observer
ax.plot(observer[0], observer[1], 'ko', markersize=8, label="Observer")

# Plot points s1 and s2
ax.plot(s1[0], s1[1], 'ko', markersize=8)
ax.plot(s2[0], s2[1], 'ko', markersize=8)

# Dashed line for theta
ax.plot([0.4, observer[0]], [0, observer[1]], 'k:', lw=2)

# Angle arc for theta
theta_arc = np.linspace(0, theta, 20)
arc_x = 0.05 * np.cos(theta_arc) + 0.4
arc_y = 0.05 * np.sin(theta_arc)
ax.plot(arc_x, arc_y, 'k')

# Labels
ax.text(-0.4, -0.15, 'RG', color='black', fontsize=12, ha='center')
ax.text(0.4, -0.12, 'WD', color='black', fontsize=12, ha='center')
ax.text(0.1, -0.02, "Accretion Flow", fontsize=10, ha='center')
ax.text(-0.6, 0.3, "RG Wind", fontsize=10, ha='center')
ax.text(s1[0] + 0.02, s1[1], r"$s = s_1$", fontsize=14, ha='left')
ax.text(s2[0] - 0.05, s2[1], r"$s = s_2$", fontsize=14, ha='right')
ax.text(observer[0] + 0.02, observer[1], "Observer", fontsize=12, ha='left')
ax.text(0.55, 0.02, r"$\theta$", fontsize=14)
